...
By default, routing simulation uses a **cheaper sim model** auto-derived from the primary model (e.g. `claude-sonnet` → `claude-haiku`, `gpt-4o` → `gpt-4o-mini`). This reduces total API cost by 80–90% since simulation is the highest-volume operation. The primary model is only used for adversarial prompt generation and description rewrites. Use `--sim-model` to override.
