...
The simulation mirrors how a real skill router works — same prompt format, same decision process.

By default, the **primary model** (`--model`) is used for routing simulation (the model under test), while a **cheaper auto-derived model** handles prompt generation and test data (e.g. `claude-sonnet` → `claude-haiku`, `gpt-4o` → `gpt-4o-mini`). This reduces total API cost since prompt generation is high-volume but doesn't need the model under test. Use `--gen-model` to override the auto-derived generation model.
