...
describe("PairCache.hashPair", () => {
  ...
  it("changes when routingModel changes", () => {
    const h1 = PairCache.hashPair("a", "descA", "b", "descB", 10, "haiku");
    const h2 = PairCache.hashPair("a", "descA", "b", "descB", 10, "gpt-4o-mini");
    expect(h1).not.toBe(h2);
  });

  it("changes when evaluation scope changes", () => {
    const h1 = PairCache.hashPair("a", "descA", "b", "descB", 10, "haiku", "audit:pair");
    const h2 = PairCache.hashPair("a", "descA", "b", "descB", 10, "haiku", "audit:fleet:manifest");
    expect(h1).not.toBe(h2);
  });
});
...
describe("PairCache get/set", () => {
  ...
  it("stores and retrieves an entry", () => {
    ...
    cache.set(hash, { promptsPerPair: 10, routingModel: "claude-haiku", report });
    ...
    expect(retrieved!.routingModel).toBe("claude-haiku");
  ...
  it("creates the cache directory automatically on set", () => {
    ...
    cache.set(hash, { promptsPerPair: 10, routingModel: "haiku", report });
...
describe("PairCache TTL expiry", () => {
  it("returns null for expired entries and removes the file", () => {
    ...
    const report = makeReport("a", "b");
    cache.set("expired-hash", { promptsPerPair: 10, routingModel: "haiku", report });
  ...
  it("returns entry within TTL", () => {
    ...
    const report = makeReport("a", "b");
    cache.set("fresh-hash", { promptsPerPair: 10, routingModel: "haiku", report });
...
describe("PairCache.clear", () => {
  ...
  it("removes all entries and returns count", () => {
    const cache = new PairCache(testDir);
    cache.set("a", { promptsPerPair: 10, routingModel: "h", report: makeReport("a", "b") });
    cache.set("b", { promptsPerPair: 10, routingModel: "h", report: makeReport("c", "d") });
    cache.set("c", { promptsPerPair: 10, routingModel: "h", report: makeReport("e", "f") });
