...
import { saveRun, enforceRetention, loadRun } from "../src/reporting/storage.js";
...
describe("loadRun", () => {
  it("loads the latest run", () => {
    const meta = makeMeta({ command: "lint" });
    const data = makeLintData();
    saveRun("lint", meta, data, testDir);

    const loaded = loadRun("lint", "latest", testDir);

    expect(loaded.metadata.id).toBe(meta.id);
    expect(loaded.data).toEqual(data);
  });

  it("loads a run by short ID", () => {
    const meta = makeMeta({ command: "lint" });
    const data = makeLintData();
    saveRun("lint", meta, data, testDir);

    const shortId = meta.id.slice(0, 8);
    const loaded = loadRun("lint", shortId, testDir);

    expect(loaded.metadata.id).toBe(meta.id);
  });

  it("returns the newest run when multiple exist", () => {
    const meta1 = makeMeta({ command: "lint", timestamp: "2026-01-01T00:00:00.000Z" });
    const meta2 = makeMeta({ command: "lint", timestamp: "2026-02-01T00:00:00.000Z" });
    saveRun("lint", meta1, makeLintData(), testDir);
    saveRun("lint", meta2, makeLintData(), testDir);

    const loaded = loadRun("lint", "latest", testDir);

    expect(loaded.metadata.id).toBe(meta2.id);
  });

  it("throws when command directory does not exist", () => {
    expect(() => loadRun("lint", "latest", testDir)).toThrow("No lint runs found");
  });

  it("throws when run ID is not found", () => {
    const meta = makeMeta({ command: "lint" });
    saveRun("lint", meta, makeLintData(), testDir);

    expect(() => loadRun("lint", "nonexistent", testDir)).toThrow('Run "nonexistent" not found');
  });
});
