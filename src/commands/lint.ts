...
export async function lintCommand(options: LintOptions): Promise<void> {
  ...
  const genModel = options.genModel ?? config.genModel;
...
async function runFleetLint(
  ...
  const dual = createDualProviders(ctx.model, ctx.config, ctx.genModel);
  const { primaryTracked, genTracked } = dual;
  const genLabel =
    dual.genModel !== ctx.model ? `${dual.genModel} (${dual.genModelSource})` : ctx.model;
  console.log(
    chalk.dim(`Sim: ${ctx.model} | Gen: ${genLabel} | Prompts/pair: ${ctx.promptsPerPair}\n`)
  );
  ...
  if (dual.genModel !== ctx.model) collector.setGenModel(dual.genModel);

  const generator = new PromptGenerator({ provider: genTracked, model: dual.genModel });
  const simulator = new RoutingSimulator({
    provider: primaryTracked,
    model: ctx.model,
...
async function runPairLint(options: LintOptions, skills: Skill[], ctx: LintContext): Promise<void> {
  ...
  console.log(
    chalk.dim(
      `Threshold: ${ctx.threshold}% | Sim: ${ctx.model} | Gen: ${genLabel} | Prompts/pair: ${ctx.promptsPerPair}\n`
    )
  );
...
async function testPairs(
  ...
  let errorCount = 0;
  let cacheHits = 0;
  const effectiveRoutingModel = routingModel ?? ctx.model;
