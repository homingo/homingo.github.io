...
export function scorePair(
  ...
  const thirdSkillHijacks = misroutes.filter(
    (d) => d.selectedSkill !== skillA && d.selectedSkill !== skillB
  );
  const hijackCounts = new Map<string, number>();
  for (const hijack of thirdSkillHijacks) {
    hijackCounts.set(hijack.selectedSkill, (hijackCounts.get(hijack.selectedSkill) || 0) + 1);
  }
  const topHijackingSkills = [...hijackCounts.entries()]
    .sort((a, b) => b[1] - a[1])
    .slice(0, 5)
    .map(([name]) => name);
  ...
  return {
    ...
    topFailurePattern,
    recommendedAction,
    thirdSkillHijackCount: thirdSkillHijacks.length,
    thirdSkillHijackRate:
      decisions.length > 0
        ? Math.round((thirdSkillHijacks.length / decisions.length) * 1000) / 10
        : 0,
    topHijackingSkills,
  };
}
...
export function scoreFleet(
  ...
  totalSkills: number,
  modelUsed: string,
  modeOrExportPath: "pair" | "fleet" | string,
  exportPath?: string
): FleetAuditReport {
  const mode =
    modeOrExportPath === "pair" || modeOrExportPath === "fleet" ? modeOrExportPath : "pair";
  const resolvedExportPath =
    modeOrExportPath === "pair" || modeOrExportPath === "fleet"
      ? (exportPath ?? "")
      : modeOrExportPath;
  ...
  const hijackCounts = new Map<string, number>();
  let totalThirdSkillHijacks = 0;
  for (const report of pairReports) {
    totalThirdSkillHijacks += report.thirdSkillHijackCount || 0;
    for (const skill of report.topHijackingSkills || []) {
      hijackCounts.set(skill, (hijackCounts.get(skill) || 0) + 1);
    }
  }
  const topHijackingSkills = [...hijackCounts.entries()]
    .sort((a, b) => b[1] - a[1])
