...
export interface PairConflictReport {
  ...
  thirdSkillHijackCount?: number;
  thirdSkillHijackRate?: number;
  topHijackingSkills?: string[];
}
...
export interface HomingoConfig {
  ...
  openaiApiKey?: string;
  model: string;
  /** Optional override for the model used for prompt generation (test data). Auto-derived if not set. */
  genModel?: string;
  skillsDir: string;
  shadowRouter: {
