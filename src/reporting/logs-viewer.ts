...
function getResultSummary(m: RunMetadata): string {
  ...
  switch (r.type) {
    case "audit":
      return `${r.mode} mode, ${r.fleetErrorRate}% error rate, ${r.criticalCount} critical`;
    case "lint":
    ...
    case "map":
      return `${r.clusters} clusters, ${r.mergeCandidates} merge candidates, ${r.overlapHubs} hubs`;
  }
}
