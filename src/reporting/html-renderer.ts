...
import type { MapData } from "../commands/map.js";
...
<header>
  ...
  <div class="meta-bar">
    ...
    ${meta.genModel && meta.genModel !== meta.model ? `<span class="meta-item"><strong>Gen:</strong> ${esc(meta.genModel)}</span>` : ""}
...
<div class="summary-cards">
  <div class="card card-stat">
    ...
    <div class="card-value">${esc(report.mode)}</div>
  </div>
  <div class="card card-stat">
  ...
  ${
    report.mode === "fleet"
      ? `<div class="card card-stat">
    <div class="card-label">Third-skill Hijacks</div>
    <div class="card-value">${report.totalThirdSkillHijacks ?? 0}</div>
  </div>`
      : ""
  }
</div>`;
  ...
  if (report.mode === "fleet") {
    html += `<section>
      <h2>Fleet Mode Notes</h2>
      <p>These prompts were generated around risky skill boundaries, then routed against the full manifest to detect third-skill hijacks and discoverability failures.</p>
    </section>`;
  }
  ...
  if (report.mode === "fleet" && (report.topHijackingSkills?.length ?? 0) > 0) {
    html += `<section><h2>Top Hijacking Skills</h2><ol>`;
    for (const name of report.topHijackingSkills || []) {
      html += `<li><strong>${esc(name)}</strong></li>`;
    }
    html += `</ol></section>`;
  }
...
<div class="summary-cards">
  ...
  <div class="card card-stat">
    ...
    <div class="card-value">${data.clusters.length}</div>
  </div>
  <div class="card card-stat">
    <div class="card-label">Merge Candidates</div>
    <div class="card-value">${data.mergeCandidates.length}</div>
  </div>
  <div class="card card-stat">
    <div class="card-label">Overloaded Skills</div>
    <div class="card-value">${data.overloadedSkills.length}</div>
  </div>
</div>`;

  html += `<section><h2>Capability Clusters <span class="count">(${data.clusters.length})</span></h2>`;
  if (data.clusters.length === 0) {
    html += `<p>No clusters detected.</p>`;
  } else {
    html += `<table>
      <thead><tr><th>Cluster</th><th>Size</th><th>Keywords</th><th>Skills</th></tr></thead>
      <tbody>`;
    for (const cluster of data.clusters) {
      html += `<tr>
        <td><strong>${esc(cluster.id)}</strong><br><span class="muted">${esc(cluster.label)}</span></td>
        <td>${cluster.size}</td>
        <td>${esc(cluster.representativeKeywords.join(", "))}</td>
        <td>${esc(cluster.skills.join(", "))}</td>
      </tr>`;
    }
    html += `</tbody></table>`;
  }
  ...
  if (data.mergeCandidates.length > 0) {
    html += `<section><h2>Merge Candidates</h2><table>
  ...
  if (data.overloadedSkills.length > 0) {
    html += `<section><h2>Overloaded Skills</h2><table>
  ...
  html += `<section><h2>Skill Profiles</h2><table>
    <thead><tr><th>Skill</th><th>Cluster</th><th>Neighbors</th><th>Notes</th></tr></thead><tbody>`;
  for (const profile of data.skillProfiles) {
    const neighbors = profile.nearestNeighbors
