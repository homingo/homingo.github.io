...
/**
 * Load a previously saved run by ID (short hash) or "latest".
 * Looks in the command subdirectory for matching JSON files.
 */
export function loadRun(
  ...
  if (!existsSync(commandDir)) {
    throw new Error(`No ${command} runs found in ${reportDir}`);
  }
  ...
  if (jsonFiles.length === 0) {
    throw new Error(`No ${command} runs found`);
  }

  let targetFile: string | undefined;
  if (runIdOrLatest === "latest") {
    targetFile = jsonFiles[jsonFiles.length - 1];
  } else {
    targetFile = jsonFiles.find((f) => f.includes(`-${runIdOrLatest}.json`));
    if (!targetFile) {
      targetFile = jsonFiles.find((f) => f.includes(runIdOrLatest));
    }
  }

  if (!targetFile) {
    throw new Error(
      `Run "${runIdOrLatest}" not found. Available runs:\n` +
        jsonFiles.map((f) => `  ${f}`).join("\n")
    );
  }

  const jsonPath = join(commandDir, targetFile);
  const raw = readFileSync(jsonPath, "utf-8");
  return JSON.parse(raw) as StoredRunData;
}
