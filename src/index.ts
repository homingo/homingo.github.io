...
program
  ...
  .option("--mode <mode>", "Audit mode: pair or fleet", "pair")
  .option("--all-pairs", "Test every skill pair (expensive, thorough)")
  .option("--prompts <count>", "Prompts per pair (default: 10)", "10")
  .option("--model <model>", "Model to use for routing simulation (the model under test)")
  .option(
    "--gen-model <model>",
    "Model for prompt generation / test data (default: auto-derived cheaper model)"
  )
...
program
  .command("map")
  .description("Capability map — cluster skills, find overlap hubs, and export a handbook view")
  .option("--skills-dir <dir>", "Path to skills directory (overrides config)")
  .option("--enhanced", "Use bigram matching for improved overlap detection")
