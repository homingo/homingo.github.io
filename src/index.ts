...
program
  ...
  .option("--prompts <count>", "Prompts per pair (default: 50)", "50")
  .option("--model <model>", "Model to use for routing simulation")
  .option("--json", "Output JSON only (no terminal table)")
  .option("--concurrency <n>", "Max parallel API calls (default: 10)", "10")
...
program
  ...
  .option("--json", "Output JSON only")
  .option("--dry-run", "Show pairs that would be tested without making API calls")
  .option("--fix", "Apply suggested rewrites and shard plans to SKILL.md files")
